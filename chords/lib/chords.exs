...
defmodule Chords1 do
    ...
    * k - from 0 to (m - 1)
    """
    def calcfinger(currentnode, k, m) do
    ...
    def createFingerTables(pidHashMap, numNodes) do
        ...
        Enum.map(0..length(pidHashMap)-1, fn(n) ->
            ...
            Enum.each(0..(m), fn(i) -> 
                ...
                nextFinger = calcfinger(currentnode, i, m)
    ...
    def lookup(currentNode , keyList, totalCount, startNode) do
        ...
        if isPresent do
            ...
            largerFingers = Enum.map(0..length(fingers)-1, fn(x)->
                if Enum.fetch!(fingers, x)[:nextFinger] > generateHash(key) do
                    x
                end
            end)
            largerFingersList = Enum.reject(largerFingers, &is_nil/1)
            if length(largerFingersList) > 0 do
                x =  Enum.fetch!(largerFingersList,0)
                newSuccessor = Enum.fetch!(fingers, x)[:successor]
                successorId = elem(newSuccessor, 0)
                {_, _, _, keysNew, _} = getState(successorId)
                if length(keysNew) > 0 do
                    ...
                    if isPresent do
                        ...
                        #     lookup(prevFinger, [key], totalCount, startNode)
                        # end
                        lookup(newSuccessor, [key], totalCount, startNode)
                    end
                else
                    # if x == 0 do
                    #     prevFinger = Enum.fetch!(fingers, length(fingers)-1)[:successor]
